...
function ServicesView() {
  ...
  function Values({ IconComponent, name, description }) {
    return (
      <div key={name} className="services__value">
        {IconComponent}
        <h3>{name}</h3>
        <p>{description}</p>
      </div>
  ...
  return (
    <section className="services">
      ...
      <section className="services__values">
        {valuesArray.map((valueObject) => Values(valueObject))}
      </section>
    </section>
  );
}
